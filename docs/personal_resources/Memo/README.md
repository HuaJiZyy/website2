...
### 普通模式
普通模式是默认的模式，你可以使用各种命令来导航、编辑和操作文本。以下是一些常用的 Vim 普通模式命令：
- `h`：向左移动光标
- `j`：向下移动光标
- `k`：向上移动光标
- `l`：向右移动光标
...
- `gg`：移动到文件首
- `G`：移动到文件尾
以上命令在可视行模式下也可以用来选择文本.
- `:` 进入命令行模式，可以输入各种命令，例如 `:w` 保存文件，`:q!` 不保存退出，`:wq` 保存并退出等。

### 插入模式(INSERT)
进入插入模式：在普通模式下按 i 键进入插入模式。可以添加、修改文本。
...
在可视行模式下，你可以进行一些操作，例如删除(按 `d`)、复制（`y`/`yy`(复制一整行)）、剪切（`d`/`dd`(剪切一整行)）、粘贴（`p`）、改变大小写（`~`）、搜索（`/` 或 `?`）等。

> 替换整个文件的内容: 按 gg 跳到文件开头, 按 dG 删除所有内容, 按 i 进入插入模式(这一步可以省略), 最后command+v粘贴即可.
