...
扩展欧几里得定理（Extended Euclidean Algorithm）是欧几里得算法的一个扩展，用于求解贝祖等式（Bézout's identity）。贝祖等式是形如 ax + by = gcd(a, b) 的线性方程，其中 a 和 b 是已知的整数，x 和 y 是需要求解的整数，gcd(a, b) 是 a 和 b 的最大公约数。特殊地, 当 x 和 y 互质时, 等式右部等于1.
...
2436x + 13y = 1 (1)
  13x +  5y = 1 (2)
   ...
   2x +   y = 1 (5)
    x +  0y = 1 (6)
(6)式中令 x=1 y=0, 即:
x'= 1 y'= 0, 回溯(5), 得: x=y'= 0 y=x'-[a/b]*y'=  1
